...
class PlanningAgent(BaseAgent):
    ...
    async def _create_project_plan(self, parameters: Dict[str, Any]) -> Dict[str, Any]:
            ...
            for component in components:
                ...
                if subtasks and component.get("sequential", True):
                    subtask.dependencies.append(
                        TaskDependency(task_id=subtasks[-1].id, type="finish_to_start")
                    )
    ...
    async def _update_project_plan(
        self, session_id: UUID, parameters: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if plan.tasks:
            plan.start_date = min(t.start_time or plan.start_date for t in plan.tasks)
            plan.end_date = max((t.end_time or plan.end_date) for t in plan.tasks)
    ...
    async def _analyze_dependencies(self, session_id: UUID) -> Dict[str, Any]:
            ...
            return {
                ...
                "dependency_stats": {
                    "total_dependencies": sum(
                        len(node["dependencies"]) for node in dependency_graph.values()
                    ),
    ...
    async def _analyze_task_complexity(self, task: PlannedTask) -> float:
        ...
        factors = [
            ...
            (task.estimated_duration.total_seconds() / (8 * 3600))
            * 0.3,  # Normalize to 8-hour days
    ...
    async def _find_critical_path(
        self, dependency_graph: Dict[UUID, Dict]
    ) -> List[UUID]:
        ...
        for task_id in self._topological_sort(dependency_graph):
            ...
            for dep_id in node["dependencies"]:
                ...
                dep_end = (
                    earliest_start[dep_id] + dep_task.estimated_duration.total_seconds()
                )
        ...
        latest_start = {}
        max_duration = max(
            earliest_start[tid] + task_durations[tid] for tid in earliest_start
        )
        critical_path = []
    ...
    async def _level_resources(self, tasks: List[PlannedTask]):
        ...
        for resource_type, resource_tasks in resource_groups.items():
            ...
            sorted_tasks = sorted(
                resource_tasks, key=lambda t: (-t.priority, len(t.dependencies))
            )
    ...
    async def _optimize_schedule(self, plan: ProjectPlan):
        ...
        try:
            ...
            for task in sorted_tasks:
                ...
                for dep in task.dependencies:
                    dep_task = next(
                        (t for t in plan.tasks if t.id == dep.task_id), None
                    )
                    if dep_task and dep_task.id in task_times:
                        dep_start, dep_end = task_times[dep_task.id]
                        if dep.type == "finish_to_start":
                            earliest_start = max(earliest_start, dep_end + dep.lag)
                        elif dep.type == "start_to_start":
                            earliest_start = max(earliest_start, dep_start + dep.lag)
    ...
    def _topological_sort_tasks(self, tasks: List[PlannedTask]) -> List[PlannedTask]:
        ...
        # Build dependency graph
        graph = {
            task.id: {"task": task, "dependencies": set(), "dependents": set()}
            for task in tasks
        }
    ...
    def _topological_sort(self, graph: Dict[UUID, Dict]) -> List[UUID]:
    ...
    async def _calculate_max_depth(self, dependency_graph: Dict[UUID, Dict]) -> int:
        ...
        return max(calculate_depth(node_id) for node_id in dependency_graph)
                        ...
                        conflicts.append(
                            {
                                ...
                                "end": min(
                                    sorted_usage[i]["end"], sorted_usage[j]["end"]
                                ),
                                "total_amount": (
    ...
    async def export_plan(
        self, session_id: UUID, format: str = "json"
    ) -> Dict[str, Any]:
