...
class StateManager:
    ...
    async def get_state(self, key: str, default: Any = None) -> Optional[Any]:
        """
        Get state value.
    ...
    async def bulk_set_state(
        ...
        try:
            ...
            for key, value in states.items():
                ...
                if not temporary:
                    await pipe.set(
                        f"state:{key}", entry.json(), ex=None if not temporary else 3600
                    )
    ...
    async def update_metadata(self, key: str, metadata: Dict) -> bool:
