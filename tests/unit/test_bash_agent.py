...
async def test_command_timeout(bash_agent):
    ...
    assert result['result']['exit_code'] != 0
    assert result['result']['terminated'], "The command should have a terminated flag set"
...
async def test_concurrent_commands(bash_agent):
    ...
    tasks = [
        bash_agent.execute_task({'command': cmd})
        for cmd in commands
    ]
...
async def test_command_termination(bash_agent):
    ...
    result = await task
    assert not result['success'], "The command should not have succeeded after termination"
